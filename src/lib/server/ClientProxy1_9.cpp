...
void ClientProxy1_9::gesture(const GestureEvent &event)
{
  LOGC(
      ...
      (CLOG_INFO "gesture.protocol send target=\"%s\" type=%d phase=%d fingers=%d delta=%d,%d sequence=%u",
       getName().c_str(), static_cast<int>(event.type), static_cast<int>(event.phase), event.fingers, event.deltaX,
